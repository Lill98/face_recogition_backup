...
class Inference():
    ...
    def load_model(self, path):
        ...
        if self.device == "cpu":
            statedict = torch.load(path, map_location=torch.device('cpu'))['state_dict']
        else:
    ...
    def transform_image(self, path_images, batch=2):
        ...
        for idx, path_image in tqdm.tqdm(enumerate(path_images)):
            ...
            try:
                img = align.get_aligned_face(path_image, "cpu")
            except:
                print("can't detect face", path_image)
                continue
    ...
    def infer(self,list_path_images):
        batch_image, list_name_folder, list_all_image = self.transform_image(list_path_images)
        ...
        try:
            torch.cat(list_embedding, dim=0)
        except:
            print("list_embedding", list_embedding)
